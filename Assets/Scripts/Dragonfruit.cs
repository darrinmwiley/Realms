...
public class Dragonfruit : MonoBehaviour
{
    ...
    public float separation = .0001f;

    public int numSegments = 2;

    public class Segment{
        public GameObject body;
        //this will be a small sphere that holds the joint so we can resize body separately
        //body will be a child of this
    ...
    public void SetBase(GameObject b)
    {
        baseObj = b;
    }
    ...
    public void AddSeed()
    {
        AddSegment(new Vector3(.01f,.01f,.01f), new Vector3(.01f,.01f,.01f), -1);
        segments[segments.Count - 1].UpdateMesh(green);
    }
    ...
    void AddSegment(Vector3 initialScale, Vector3 finalScale, float growTime)
    {
        ...
        Vector3 tip = baseObj.transform.position + Vector3.up * baseObj.transform.localScale.y / 2f;
        ...
        Vector3 newPosition = tip + rotation * Vector3.up * (initialScale.y);
        Debug.Log(tip+" "+newPosition);
    ...
    ConfigurableJoint ConfigureJoint(GameObject obj, GameObject connected)
    {
        ...
        jointLimit.limit = 5; // 15 degrees limit
    ...
    float lastSegmentAdded = 0;

    void Update()
    {
        if(Time.time - lastSegmentAdded > 6)
        {
            AddSegment(new Vector3(.02f, .02f, .02f),new Vector3(.2f, .2f, .2f),-1);
            lastSegmentAdded = Time.time;
        }
